...
For accurate timings, install the package from an optimized build. If you ran `roxygen2::roxygenise()` first, run `./cleanup` and reinstall with `R CMD INSTALL --configure-args='--without-cuda' .` before benchmarking. Better still, use `R CMD build` and install from the tarball.
